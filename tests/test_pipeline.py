...
from flight_radar.providers.serpapi_openjaw import OUTBOUND_FOLLOWED
...
DAYS_PER_MONTH = 31
CALLS_PER_PAIR = 1 + OUTBOUND_FOLLOWED   # 가는 편 목록 1회 + 후보별 오는 편
...
def test_the_metered_routes_stay_inside_the_free_search_quota():
    ...
    Several searches per date pair, once a day: the first response describes
    only the flight out, so every outbound candidate we follow costs another
    call. Quota exhaustion would show up as silently missing open-jaw prices,
    which nothing else in the pipeline would notice. Widening the window, or
    following more outbounds, breaks this - that is the point.
    """
    routes = load_routes(Paths(_repo_root()).routes)
