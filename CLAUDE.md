...
**2026-09-10 구매 완료**: 리스본 in / 마드리드 out, **에티하드 Value 161만원**
(10/05 ICN 18:20 → LIS / 10/15 MAD 21:50 → ICN, 위탁 25kg 포함).
**자동 수집은 중단했다** — 두 워크플로의 `schedule` 을 주석 처리했고
`workflow_dispatch` 만 남겼다. 다시 쓰려면 주석을 풀면 된다.

**운임 등급을 보지 않으면 비교가 틀린다.** 도구가 추적한 값은 전부
**Basic 운임(위탁수하물 0kg)** 이었다. Google 도 SerpApi 도 수하물 정보를 주지
않으므로 이건 도구로 알 수 없다. Basic 155.3만 + 수하물 21.8만(20kg) = 177.1만
보다 Value 161만(25kg)이 쌌다. 다음에도 **예매 화면에서 운임 등급을 비교**할 것.

**Google `price_insights` 는 왕복 조회에만 붙는다** (다구간엔 없음). 오픈조가
적정가인지 볼 때 우회 근거가 된다. 2026-09-09 실측: ICN↔LIS 왕복 10/05-10/15 이
`price_level: high`, 통상 105~180만, 현재 201.6만 — 오픈조 155만은 범위 안이었다.
...
**매일 × 1조합 × 6쌍 = 월 186회**에 맞췄고, `window.depart_until`로 출발일을
...
대신 대시보드는 **우리가 직접 쌓은 관측**으로 곡선을 그린다
(`dashboard.observed_lows`). Google 곡선과 섞지 않는다 — 그쪽은 일별·조밀,
이쪽은 수집 1회당 1점이라 같은 측정이 아니다. 눈금도 'N일 전'이 아니라
실제 날짜로 찍는다.
...
**단 최저 경신은 구간으로 묶지 않는다.** 2026-09-08 사고: 9/3에 139.9만 목표가
알림이 나간 뒤 9/8에 136.5만으로 최저를 깼는데, 같은 27번 구간이라 7일 차단에
걸려 **조용히 사라졌다.** 직전 최저를 깬 알림은 이유와 무관하게 정확한 가격으로
기록한다 — 최저는 정의상 이전 모든 값보다 낮으므로 중복될 수 없다.
10% 미달이라 `drop`도 못 되던 구간을 위해 `low` 이유도 더했다
(마진 1만원 — 몇 천 원짜리 갱신까지 알리면 결국 알림을 끄게 된다).
